import type { AnonymizationMode, Finding, PIIType, RiskLevel } from "./types.js";
...
// ---------------------------------------------------------------------------
function hasBankingContext(text: string, start: number, end: number): boolean {
  const window = 60;
...
function overlapsExisting(findings: Finding[], start: number, end: number): boolean {
